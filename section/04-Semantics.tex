...
\\ \>                   \> ~|  \> $\kSet~ Type~ |~ \kSets~ Type~ |~ \kFact~ Type~ |~ \kFACT$
...
Our type language is standard. In Figure~\ref{f:Grammar} type ()$@Sets@~ Type$) classifies multisets. The type ($@Fact@~ Type$) classifies fact values whose payload has type $Type$. The @FACT@ type is used as the supertype of all such fact types, omitting a parameter for the payload type so that rule bodies can produce sets of factoids of differing sorts.
...
% \Amos: I put quotes around "sees": I read it as a regular verb at first. Otherwise maybe a different font for it
